...
<div class="content-section" id="comunicados">
    <div class="header">
        ...
        <div class="search-bar">
            <form method="GET">
                <input type="text" name="search" placeholder="Buscar comunicado..." value="<?= isset($_GET['search']) ? htmlspecialchars($_GET['search']) : '' ?>">
    ...
    <div class="table-container">
        ...
        <table>
            ...
            <tbody>
                <?php
                $pdo = $instituicaoObj->getPdo();

                if (!empty($_GET['search'])) {
                    $data = $_GET['search'];
                    $sql = "SELECT * FROM comunicado 
                            WHERE titulo LIKE :search 
                               OR conteudo LIKE :search 
                               OR destinatarios LIKE :search
                            ORDER BY id DESC";
                    $stmt = $pdo->prepare($sql);
                    $param = "%$data%";
                    $stmt->bindParam(':search', $param);
                    ...
                    $sql = "SELECT * FROM comunicado WHERE instituicao_id = :instituicao_id ORDER BY id DESC";
                    $stmt = $pdo->prepare($sql);
                    $stmt->bindParam(':instituicao_id', $_SESSION['id'], PDO::PARAM_INT);
                    $stmt->execute();
                    $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
                }

                if (!empty($result)):
                    foreach ($result as $comunicado):
                ?>
                    <tr>
                        <td><?= htmlspecialchars($comunicado['titulo']) ?></td>
                        <td><?= htmlspecialchars($comunicado['destinatarios']) ?></td>
                        <td><?= nl2br(htmlspecialchars($comunicado['conteudo'])) ?></td>
                        
                        <td>Enviado</td>
                        <td>
                            <a href="editarComunicado.php?id=<?= $comunicado['id'] ?>">Editar</a> |
                            <a href="deletarComunicado.php?id=<?= $comunicado['id'] ?>" onclick="return confirm('Deseja realmente excluir este comunicado?')">Excluir</a>
                        </td>
                    </tr>
                <?php
                    endforeach;
                ...
                ?>
                    <tr>
                        <td colspan="6" style="text-align: center;">Nenhum comunicado encontrado.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
                ...
                <div class="table-container">
                    ...
                    <form id="form-comunicado" action= "../controller/inserirComunicado.php" method= "POST">
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="submit" class="btn-save" name="btn-Comunic" id="btn-Comunic">Enviar Comunicado</button>
                        </div>
                    </form>
                </div>
            </div>
